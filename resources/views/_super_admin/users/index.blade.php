    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between py-4 gap-4">
        <div>
            ...
            <p class="text-md text-gray-400 dark:text-neutral-400">
                Kelola data pengguna aplikasi sekolah.
            </p>
        </div>
    ...
    <div class="px-2 pt-0 mb-4">
        <form action="{{ route('superadmin.users.index') }}" method="GET" navigate-form
                ...
                <div class="relative">
                    <input type="text" name="keywords" id="keywords" value="{{ $keywords ?? '' }}"
                        class="py-1 px-3 block w-full border-gray-200 rounded-lg text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900
                            placeholder-neutral-300 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" placeholder="Cari Nama ">
                </div>
            </div>
            <div class="sm:w-64">
            ...
            <div>
                ...
                @if (!empty($keywords) || (!empty($school_id) && $school_id !== 'all'))
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-neutral-700/50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-x-3">
                                    <div
                                        class="shrink-0 size-10 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 font-bold dark:bg-blue-900/30 dark:text-blue-400">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm text-gray-800 dark:text-neutral-200">
                                    @php
                                        $userSchool = $schools->where('id', $d->school_id)->first();
                                    @endphp
                                    {{ $userSchool->school_name ?? 'Super Admin' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
    ...
    <script>
        function setDeleteData(id, name) {
          ...
          function setResetPasswordData(id, name) {
            ...
            document.getElementById('reset-form').action = '{{ url('superadmin/users/reset-password') }}/' + id;
        }
    </script>
@endsection