...
<div class="grid gap-3 md:flex md:justify-between md:items-center py-4">
    <x-page-title title="Data {{ $page['title'] }}" description="Manajemen Data Sekolah" />
</div>
...
<script>
    function setDeleteData(id, name) {
        ...
        superadmin / schools / delete ') }}/' + id;
    }

    function setRestoreData(id, name) {
        document.getElementById('restore-item-name').textContent = name;
        let url = "{{ route('superadmin.schools.restore', ':id') }}";
        url = url.replace(':id', id)
        document.getElementById('restore-form').action = url;
    }
</script>
@endsection
